...
export default function MessBill() {
  ...
  const fetchBillData = async (regNo, month, year) => {
    ...
    try {
      const response = await fetch(
        `http://localhost:4000/api/bill/${regNo}?month=${month}&year=${year}`
      );
      if (!response.ok) {
  ...
  const fetchBillHistory = async (regNo) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/bill/history/${regNo}`
      );
      if (!response.ok) {
          ...
          {loading ? (
            ...
            <div className="bill-details">
              ...
              <div className="payment-status">
                <div
                  className={`status ${billData.isPaid ? "paid" : "unpaid"}`}
                >
              ...
              <div className="bill-notes">
                <p>
                  Note: Your bill is calculated based on days you were marked
                  "in" the mess.
                </p>
